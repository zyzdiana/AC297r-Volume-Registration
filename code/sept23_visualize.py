...
def load_pickle(res,rotation,path = '/Users/zyzdiana/Dropbox/THESIS/'):
    filename1 = path + 'sept23_tricubic_%s_%s_rotation_001_0.p' % (res, rotation)
    cost_dict_1 = pickle.load(open(filename1,'rb'))
    filename2 = path + 'sept23_tricubic_%s_%s_rotation_001_1.p' % (res, rotation)
    cost_dict_2 = pickle.load(open(filename2,'rb'))
    return preprocess(cost_dict_1,cost_dict_2)

def split_plot(cost_dict_1, cost_dict_2,cost_dict_1_shifted, cost_dict_2_shifted,cost_dict_1_filtered, cost_dict_2_filtered,res,ax_to_idx_dict):
...
def scatter_plot_trans(cost_dict, res, axes):
    ...
    plt.ylim([0,6])
    plt.legend(loc='best')
    plt.xlabel('True Translations (mm)', fontsize = 15)
    plt.ylabel('Search Results from Registration (mm)', fontsize = 15)
    plt.title('Search Results for %s Resolution' %res, fontsize = 15)
...
def scatter_plot_all_trans(axes, cost_dict_10, cost_dict_8, cost_dict_6_4,figsize = [12,4], interp = 'Linear'):
    ...
    #plt.suptitle('%s Interpolation'% interp, fontsize = 20)
    plt.show() 
...
def error_plot_trans(cost_dict, col, res):
    ...
    for idx, rot_ax in enumerate(axes_dict.keys()):
        ...
        for ii, rot_range in enumerate(ranges):
            ...
            plt.ylim([-2.0,1.0])
            #plt.title('%s, %s, rot_%s, trans_%s' % (res,rot_range,rot_ax,axes_dict[rot_ax]))
            plt.title('Error Plot for Translations',fontsize=20)
            #plt.xlabel('Data Set')
